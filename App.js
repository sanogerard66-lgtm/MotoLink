...
import {
  ...
  ActivityIndicator, Alert, ScrollView, Linking, Keyboard,
  BackHandler, Animated, Dimensions, StatusBar, Platform,
  Modal, Image, Easing, AppState, PanResponder,
...
const getSurgeMultiplier = async () => {
  try {
    ...
    // Demand-based check
    const [{ count:searchingPassengers }, { count:activeDrivers }] = await Promise.all([
      supabase.from('trips').select('*',{count:'exact',head:true}).eq('status','searching'),
      supabase.from('profiles').select('*',{count:'exact',head:true}).eq('role','driver').eq('is_suspended',false),
    ]);
        ...
        <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={{flex:1}}>
          <View style={styles.modalBg}>
            ...
            <TouchableOpacity onPress={()=> { setStars(0); setReview(''); onSkip(); }} style={ { marginTop: 14 }}>
              <Text style={{ color:C.gray, fontSize:13 }}>{t.skipRating}</Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
        </KeyboardAvoidingView>
...
<body>
  ...
  <div class="section">
    ...
    <div class="row"><span class="label">Date</span><span class="value">${trip.created_at ? new Date(trip.created_at).toLocaleDateString([],{day:'2-digit',month:'long',year:'numeric'}) : '—'}</span></div>
    <div class="row"><span class="label">Time</span><span class="value">${trip.created_at ? new Date(trip.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}) : '—'}</span></div>
  ...
  const shareReceiptWhatsApp = (trip, role) => {
    ...
    const msg = encodeURIComponent(
      ...
      `📅 ${trip.created_at ? new Date(trip.created_at).toLocaleDateString([],{day:'2-digit',month:'short',year:'numeric'}) : '—'} ` +
  ...
  // TRIP HISTORY MODAL
  // ══════════════════════════════════════════════
  const TripHistoryModal = ({ visible, onClose, userId, role, t, onCompleteTrip, onCancelTrip, onConfirmPayment }) => {
    ...
    const [summary,     setSummary]     = useState({ totalEarnings:0, totalTrips:0 });
    const [histTab,     setHistTab]     = useState('active'); // 'active' | 'history'
    const PAGE_SIZE = 20;
    ...
    const loadActiveTrips = async () => {
      ...
      let q = supabase.from('trips').select('*').in('status', activeStatuses).order('created_at',{ascending:false});
    ...
    const loadTrips = async (pageNum, reset=false) => {
      ...
      if (reset && role === 'driver') {
        ...
        setSummary({ totalEarnings, totalTrips: all?.length||0 });
      }
      if (reset && role === 'passenger') {
        const { count } = await supabase.from('trips')
          .select('*',{count:'exact',head:true}).eq('passenger_id', userId).eq('status','completed');
        setSummary({ totalEarnings:0, totalTrips: count||0 });
      }
    };
  ...
  // ══════════════════════════════════════════════
  const EarningsDashboard = ({ driverId, t }) => {
    ...
    const loadEarnings = async (p) => {
      ...
      const byDay = {};
      trips.forEach(tr => {
        const day = new Date(tr.created_at).toLocaleDateString([],{weekday:'short',day:'2-digit',month:'short'});
      ...
      setData({ totalEarnings, totalTrips, avgPerTrip, peakDay, recent });
    };

    useEffect(() => { loadEarnings(period); }, [period, driverId]);
  ...
  const SOSButton = ({ onPress }) => {
    ...
    const pan     = useRef(new Animated.ValueXY({ x:20, y:height-210 })).current;
  ...
  const MorphingMenu = ({
    ...
    useEffect(()=> {
      Animated.spring(anim, {
        toValue: isOpen?1: 0, useNativeDriver: true, friction: 7
      }).start();
    }, [isOpen]);
    ...
    return (
      <TouchableOpacity onPress={onPress} style={styles.hamburgerWrap} activeOpacity={0.8}>
        ...
        <Animated.View style={[styles.bar, { transform: [{ translateY: Animated.multiply(transY, -1)}, { rotate: rotBot }]}]} />
      </TouchableOpacity>
    );
  };
  ...
  function MotoLink() {
    ...
    // Scheduled rides
    const [tripMode,         setTripMode]         = useState('now'); // 'now' | 'later'
    const [scheduledFor,     setScheduledFor]     = useState(null);
    const [showDatePicker,   setShowDatePicker]   = useState(false);
    // Multi-stop
    const [stops,            setStops]            = useState([]); // [{name,lat,lng}]
    // Package delivery
    const [serviceMode,      setServiceMode]      = useState('ride'); // 'ride' | 'delivery'
      ...
      (async () => {
        try {
          ...
          if (saved?.session && saved?.profile) {
            ...
            if (fp && !error) {
              dispatch({ type: 'SET_SESSION', p: { session: saved.session, profile: fp } });
              dispatch({ type: 'SET_ROLE', p: saved.role || fp.role });
              if (saved.lang) dispatch({ type: 'SET_LANG', p: saved.lang });
              dispatch({ type: 'SET_HYDRATED' });
              ...
              const activeStatuses = ['searching','accepted','completion_requested','awaiting_driver_confirm'];
              let tripQuery = supabase.from('trips').select('*').in('status', activeStatuses).order('created_at',{ascending:false}).limit(1);
            ...
            dispatch({ type: 'SET_HYDRATED' });
            dispatch({ type: 'SET_STEP', p: 'splash' });
          }
        } catch (e) {
  ...
  useEffect(()=> {
    const onBack = ()=> {
      ...
      if (sosModal)      { setSosModal(false); return true; }
  ...
  useEffect(() => {
    ...
    const checkScheduled = async () => {
      ...
      const { data: scheduled } = await supabase.from('trips')
    ...
    return () => clearInterval(interval);
  }, [state.step, state.role, t]);

  // ─── Load leaderboard ────────────────────
  const loadLeaderboard = async () => {
    const { data } = await supabase.from('driver_leaderboard').select('*');
  ...
  const handleSignUp = async()=> {
    ...
    if (ex) { setAuthLoading(false); return Alert.alert('Error', 'Phone already registered.'); }
    ...
    let attempts = 0;
    while (attempts < 5) {
      const { data: existing } = await supabase.from('profiles').select('id').eq('referral_code', refCode).single();
      if (!existing) break;
    ...
    if (usedRefCode) {
      const { data: referrer } = await supabase.from('profiles').select('id').eq('referral_code', usedRefCode).single();
      if (referrer) referredById = referrer.id;
    }

    const newId = uuid();
    const np2 = {
      id: newId, phone: np, password, name: nameVal,
      role: state.role, avatar: null, plate: '',
      rating: 5.0, total_ratings: 0, wallet_balance: 0,
      referral_code: refCode,
      referred_by:   referredById,
      referral_earnings: 0,
    };
    const { error } = await supabase.from('profiles').insert([np2]);
    setAuthLoading(false);
    if (error) {
      ...
      if (referredById) {
        ...
        if (referrer) {
          ...
          if (tk) await sendExpoPush(tk, '💰 Referral Bonus!', `${nameVal} used your code. 200 FRW added to your wallet.`, { type:'wallet' });
        }
      }
  ...
  const applyPromoCode = async (baseFare) => {
    ...
    setPromoLoading(false);
    setPromoData({ code, type: promo.type, value: promo.value, discount, promoId: promo.id });
    showBanner(t.promoApplied, `${fmtFRW(discount)} ${t.promoSaved}`, 'wallet');
  };

  const clearPromo = () => { setPromoData(null); setPromoCode(''); };
  ...
  const triggerSOS = async () => {
    ...
    const { data:adminTokens } = await supabase.from('profiles')
  ...
  const joinCompany = async () => {
    ...
    await supabase.from('profiles').update({ company_id: company.id, is_company_account: true })
      .eq('id', state.session.user.id);
    dispatch({ type:'SET_PROFILE', p:{ ...state.profile, company_id: company.id, is_company_account: true } });
  ...
  const captureDeliveryPhoto = async (tripId) => {
    ...
    if (status !== 'granted') { Alert.alert('Error','Camera permission required.'); return; }
    ...
    if (!result.canceled && result.assets?.[0]?.uri) {
      ...
      await supabase.from('trips').update({ delivery_photo_url: urlData.publicUrl }).eq('id', tripId);
  ...
  const checkAndAwardLeaderboard = async () => {
    ...
    const { data: leaders } = await supabase.from('driver_leaderboard').select('*').limit(1);
    if (leaders?.[0]) {
      const top = leaders[0];
      ...
      if (profile) {
        ...
        await supabase.from('profiles').update({ wallet_balance: newBal }).eq('id', top.id);
        await supabase.from('transactions').insert([{
        ...
        if (tk) await sendExpoPush(tk,'🏆 Weekly Champion!',`You were #1 this week! ${fmtFRW(LEADERBOARD_BONUS)} bonus added.`,{type:'wallet'});
      }
    }
  };
  ...
  const requestRide = async()=> {
    ...
    const snapDest  = { ...destCoords };
    const snapLoc   = { ...state.myLocation };
    ...
    if (!isOnline) {
      const queue = [...offlineQueue, tripPayload];
    ...
    const { data: tripRow, error } = await supabase.from('trips').insert([tripPayload]).select().single();
    setRideLoading(false);
    ...
    if (error) {
      ...
      if (promoData?.code && tripRow?.id) {
        ...
        await supabase.rpc('increment_promo', { code_val: promoData.code }).catch(()=>{});
      }
      // B2B monthly spend tracking
      if (state.profile?.company_id) {
        await supabase.from('companies').update({
          monthly_spend: supabase.rpc('add_spend', { cid: state.profile.company_id, amount: finalPrice })
  ...
  const requestCompletion = async()=> {
    ...
    dispatch({ type:'SET_ACTIVE_TRIP', p:{ ...state.activeTrip, status:'completion_requested' } });
    const tk = await getPushToken(state.activeTrip.passenger_id);
    await sendExpoPush(tk,'🏁 '+t.confirmComplete, t.completionRequested,{ type:'completed' });
  ...
  const markStopReached = async () => {
    ...
    await supabase.from('trips').update({ current_stop_index: nextIndex }).eq('id', state.activeTrip.id);
    dispatch({ type:'SET_ACTIVE_TRIP', p:{ ...state.activeTrip, current_stop_index: nextIndex } });
    if (allReached) {
      showBanner('✅', t.allStopsReached, 'completed');
    } else {
      const nextStop = tripStops[nextIndex];
      if (nextStop) setTargetLocation({ latitude: nextStop.lat, longitude: nextStop.lng });
  ...
  const updateDeliveryStatus = async (newStatus) => {
    if (!state.activeTrip) return;
    await supabase.from('trips').update({ status: newStatus }).eq('id', state.activeTrip.id);
    dispatch({ type:'SET_ACTIVE_TRIP', p:{ ...state.activeTrip, status: newStatus } });
    if (newStatus === 'picked_up') {
      const tk = await getPushToken(state.activeTrip.passenger_id);
      await sendExpoPush(tk,'📦 Package Picked Up!','Your package is on the way.',{ type:'accepted' });
    ...
    if (newStatus === 'delivered') {
      ...
      const tk = await getPushToken(state.activeTrip.passenger_id);
      await sendExpoPush(tk,'✅ Package Delivered!','Your package has been delivered.',{ type:'completed' });
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={['top']} style={styles.header}>
        <View style={styles.headerRow}>
          ...
          <TouchableOpacity
            style={[styles.historyBtn, { marginLeft: state.role==='driver' ? 'auto' : 6 }]}
            onPress={()=>setHistoryModal(true)}>
            <Text style={{ fontSize:18 }}>🕐</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.avatarWrap, { marginLeft: 8 }]} onPress={()=>setProfileModal(true)}>
      ...
      <Animated.View style={[styles.statusPanel, { transform: [{ translateY: menuAnim }]}]}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                {tripMode==='later' && (
                  <View style={styles.datePickerBox}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                      {Array.from({length:MAX_SCHEDULE_DAYS*24},(_,i)=>{
                    ...
                    {scheduledFor && (
                      <Text style={{color:C.gold,fontSize:12,marginTop:8,fontWeight:'700'}}>
                        📅 {scheduledFor.toLocaleDateString([],{weekday:'long',day:'2-digit',month:'short'})} · {scheduledFor.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}
                      </Text>
                    )}
                  </View>
                )}
                ...
                <View style={styles.payToggleRow}>
                  ...
                  {['cash','momo'].map(m=>(
                    <TouchableOpacity key={m} onPress={()=>setPaymentMethod(m)}
                      style={[styles.payToggleBtn,paymentMethod===m&&styles.payToggleBtnActive]}>
                      <Text style={[styles.payToggleTxt,paymentMethod===m&&{color:C.gold}]}>
                  ...
                  <View style={styles.promoRow}>
                    ...
                    <TouchableOpacity
                      ...
                      {promoLoading
                        ...
                        :<Text style={styles.promoApplyTxt}>{t.applyCode}</Text>
                      }
                    </TouchableOpacity>
                  </View>
                ) : (
                ...
                <TouchableOpacity style={styles.mainBtn} onPress={requestRide} disabled={rideLoading}>
                  {rideLoading
                    ...
                    :<Text style={styles.mainBtnTxt}>🛵 {t.req?.toUpperCase()||'REQUEST MOTO'}</Text>
                  }
                </TouchableOpacity>
              </ScrollView>
            ):(
                ...
                <View key={j.id} style={styles.jobCard}>
                  <View style={{flex:1}}>
                    ...
                    {j.is_scheduled&&j.scheduled_for&&(
                      <Text style={{color:C.purple,fontSize:11,fontWeight:'700',marginBottom:4}}>
                        📅 {t.scheduledFor}: {new Date(j.scheduled_for).toLocaleDateString([],{day:'2-digit',month:'short'})} · {new Date(j.scheduled_for).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}
                      </Text>
                    )}
                    ...
                    {j.stops&&JSON.parse(j.stops).length>0&&(
                      <Text style={{color:C.blue,fontSize:10,marginLeft:16,marginTop:2}}>+ {JSON.parse(j.stops).length} {t.stops}</Text>
                    )}
                    <View style={styles.routeLine_}/>
                  ...
                  <View style={{gap:6}}>
                    ...
                    {j.is_scheduled&&!j.pre_accepted_by&&(
                      <TouchableOpacity onPress={()=>preAcceptScheduledTrip(j.id)}
                        style={[styles.acceptBtn,{backgroundColor:'transparent',borderWidth:1,borderColor:C.purple}]}>
                        <Text style={[styles.acceptBtnTxt,{color:C.purple}]}>PRE</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  sosBtn: {
    ...
    elevation:12, shadowColor:C.red, shadowOffset:{width:0,height:4},
